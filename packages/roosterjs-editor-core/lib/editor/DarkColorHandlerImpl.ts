...
export default class DarkColorHandlerImpl implements DarkColorHandler {
    ...
    constructor(private contentDiv: HTMLElement, options?: DarkColorManagerOptions) {
        options = options || {};

        this.knownColors = options.knownColors || {};
        this.getDarkColor = options.getDarkColor;
        this.isDarkMode = !!options.inDarkMode;
    }
    ...
    adjustColors(root: HTMLElement, isCleaningUp: boolean, includeSelf: boolean) {
        ...
        this.iterateElements(root, includeSelf, element => {
            [false, true].forEach(isBackground => {
                const color = getColor(element, isBackground, this);
